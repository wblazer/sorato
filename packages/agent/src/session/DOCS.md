...
Persistent conversation storage with tree-structured history. Conversations are trees, not lists — users can fork at any point, try a different prompt, and switch between branches.
...
- `src/harness/` — produces conversations (`Prompt.Prompt`) that session storage persists
