...
- [x] **Agent tools** — file read, file edit (hashline protocol), shell exec, file search, file write. `@effect/ai Toolkit` tools that delegate to `Shell`/`Files` services.
- [x] **Sandbox** — local execution environment (`Shell` + `Files`). The harness dispatches tool calls; the sandbox executes them.
- [x] **Session storage** — tree-structured conversation persistence via SQLite. Messages have parent pointers. Forking and branch switching work at the data layer.
- [x] **Evals** — coding evals that exercise real sandbox access.
- [x] **Server + Web UI** — agent server with HTTP API. SvelteKit web UI that can create sessions, send messages, stream responses via SSE. Basic sidebar + chat layout.
...
- [ ] **Message queuing** — queue user messages when the agent is mid-loop and insert the user message as soon as possible
