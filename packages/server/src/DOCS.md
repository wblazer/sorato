...
- `session/` owns server-side persistent conversation state. Core returns lifecycle events and conversations; the server decides what to persist.
...
- `../../core/` — the agent loop, sandbox, and tools the server invokes
- `session/` — persistent conversation storage owned by the server coordinator
