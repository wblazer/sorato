<script lang="ts">
	...
	let { parts, isRunning }: { parts: MessagePart[]; isRunning: boolean } = $props();
			...
			{#each parts as part}
				{#if part.type === 'text'}
					<div class="whitespace-pre-wrap break-words font-mono text-sm leading-relaxed">
						{part.text}{#if isRunning}<span class="inline-block h-4 w-0.5 animate-pulse bg-blue-400"></span>{/if}
					</div>
