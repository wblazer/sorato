<script lang="ts">
	...
	$effect(() => {
		const unregister = hotkeyStore.register('Escape', () => {
			if (sessionStore.isRunning(sessionId) && !sessionStore.isStopping(sessionId)) {
				handleStop();
			}
		});
		return unregister;
	...
	function handleResume() {
		messagesStore.addOptimisticUserMessage(sessionId, 'Continue where you left off.');
...
<div class="flex h-full flex-col">
	...
	<div class="flex items-center gap-3 border-b border-border px-6 py-3">
		...
		{#if isStopping}
			<span class="flex items-center gap-1.5 rounded-full bg-amber-500/10 px-2 py-0.5">
	...
	{#if wasInterrupted}
		<div class="flex justify-center border-t border-border bg-background px-4 py-2">
			<button
	...
	<Composer
		...
		{isStopping}
		disabled={isRunning}
		placeholder={isStopping ? 'Stopping...' : isRunning ? 'Agent is responding...' : 'Send a follow-up message...'}
	/>
</div>
