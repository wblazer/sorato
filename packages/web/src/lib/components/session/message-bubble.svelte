...
<div class="group flex flex-col gap-2 border-l-2 {roleBorders[role] ?? 'border-l-border'} py-2 pl-4">
	<!-- Role badge + metadata -->
	<div class="flex items-center gap-2">
		<span
			class="text-[10px] font-bold uppercase tracking-widest {roleColors[role] ?? 'text-muted-foreground'}"
		>
			{role}
		</span>
