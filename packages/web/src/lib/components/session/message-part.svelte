  ...
  <details class="group">
    ...
    >
      <span class="inline-flex items-center gap-1.5">
        <span class="inline-block h-1.5 w-1.5 rounded-full bg-violet-500/60"
        ></span>
        reasoning
      </span>
    </summary>
...
{:else if part.type === 'tool-call'}
  <div class="flex flex-col gap-1.5">
    <div class="flex items-center gap-2">
      <span class="inline-block h-1.5 w-1.5 rounded-full bg-blue-500/60"></span>
    ...
    {#if part.params != null}
      <pre
        class="max-h-48 overflow-auto rounded bg-muted/50 p-2 text-xs leading-relaxed">{JSON.stringify(
          part.params,
          null,
          2
        )}</pre>
...
{:else if part.type === 'tool-result'}
  <div class="flex flex-col gap-1.5">
    <div class="flex items-center gap-2">
      <span
        class="inline-block h-1.5 w-1.5 rounded-full {part.isFailure
          ? 'bg-red-500/60'
          : 'bg-green-500/60'}"
      ></span>
      <span
        class="text-xs font-medium {part.isFailure
          ? 'text-red-400'
          : 'text-green-400'}"
...
{:else if part.type === 'file'}
  <div class="flex items-center gap-2">
    <span class="inline-block h-1.5 w-1.5 rounded-full bg-amber-500/60"></span>
